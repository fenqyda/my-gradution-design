 ...
 * 根据传入的id获取用户信息
 */
router.get('/api/user_infobyid', (req, res) => {
    // 获取参数
    let userId = req.query.user_id;

    let sqlStr = "SELECT * FROM user_info WHERE id = " + userId + "";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            res.json({ err_code: 0, message: '请求用户数据失败' });
        } else {
 ...
 *查询购物车中未付款的订单
 */
 router.get('/api/cart_goods_npay', (req, res) => {
    // 获取参数
    let user_id = req.query.user_id;
    let sqlStr = "SELECT * FROM cart WHERE user_id =" + user_id +" and is_pay = 0";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '请求购物车商品数据失败' });
 ...
 *查询购物车中已付款的订单
 */
 router.get('/api/cart_goods_payed', (req, res) => {
    // 获取参数
    let user_id = req.query.user_id;
    let sqlStr = "SELECT * FROM cart WHERE user_id =" + user_id +" and is_pay = 1";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '请求购物车商品数据失败' });
...
router.get('/api/admin_allusers', (req, res) => {
    ...
    let sqlStr = 'SELECT id, user_name, user_phone, user_nickname, user_address ,user_sex FROM user_info';
 ...
 * 获取所有订单信息
 */
router.get('/api/admin_allgoods', (req, res) => {
    // 获取参数
    let sqlStr = "SELECT goods_id, goods_name, price, buy_count, is_pay, user_name, user_phone, user_address, user_id FROM cart,user_info where cart.user_id = user_info.id";
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '请求购物车商品数据失败' });
 ...
 * 获取所有评论信息
 */
 router.get('/api/Allgoodscomment', (req, res) => {
    // 获取参数
    let sqlStr = 'SELECT comment_id, comment_detail, comment_rating, user_id, user_name, short_name FROM comments,user_info,recommend where comments.user_id = user_info.id and comments.goods_id = recommend.goods_id';
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '请求评论数据失败' });
        } else {
            results = JSON.parse(JSON.stringify(results));
            res.json({ success_code: 200, message: results });
        }
...
router.post('/api/update_recom_goods', (req, res) => {
    ...
    let sqlStr = "UPDATE recommend SET goods_name = ?, short_name = ?, price = ?, category = ? WHERE goods_id = " + goods_id;
    let strParams = [goods_name, short_name, price, category];
    conn.query(sqlStr, strParams, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '修改失败!' });
        ...
        conn.query(sql_str, (error, results, fields) => {
            if (error) {
                ...
                if (results[0]) { // 商品已经存在
                    ...
                    let add_sql = "INSERT INTO recommend(goods_id, goods_name, short_name, thumb_url, image_url, hd_thumb_url, price, normal_price, market_price, sales_tip, category, comments_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    let sql_params = [goods_id, goods_name, short_name, thumb_url, image_url, hd_thumb_url, price, normal_price, market_price, sales_tip, category];
                    conn.query(add_sql, sql_params, (error, results, fields) => {
                        if (error) {
                            console.log(error);
 ...
 * 删除单条评论消息
 */
 router.post('/api/delete_comment', (req, res) => {
    // 获取数据
    const comment_id = req.body.comment_id;

    let sqlStr = "DELETE FROM comments WHERE comment_id =" + comment_id ;
    conn.query(sqlStr, (error, results, fields) => {
        if (error) {
            console.log(error);
            res.json({ err_code: 0, message: '删除失败!' });
