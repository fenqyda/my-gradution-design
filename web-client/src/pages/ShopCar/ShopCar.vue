...
export default {
  ...
  methods: {
    ...
    updateGoodsCount(goods, count) {
      let userid = this.userInfo.id;
      this.$store.dispatch("updateGoodsCount", { goods, count, userid });
    ...
    getAllGoodsPrice() {
      ...
      this.cartgoods.forEach((goods, index) => {
        if (goods.checked) {
          totalPrice += goods.price * goods.buycount;
        }
      });
    ...
    emptyCart() {
      this.$confirm("您确定删除所有商品吗？", "提示", {
        ...
        .then(() => {
          ...
          this.$store.dispatch("delAllGoods", { userid });
          this.getAllGoodsPrice();
        })
        .catch(() => {
          this.$message({
    ...
    payment() {
      this.$confirm("您确定结算商品吗？", "提示", {
        ...
        for (let i = 0; i < this.cartgoods.length; i++) {
          if (this.cartgoods[i].checked == false || this.cartgoods[i].checked == undefined) {
            j--;
            // console.log(j);
