    ...
    <div class="user-detail-group">
      <div class="user-icon">
        ...
        <el-upload
          ...
          action="http://localhost:3000/user/upuserthumb"
...
export default {
  ...
  mounted() {
    ...
    this.user_sex = this.userInfo.sex || "";
    this.user_phone = this.userInfo.phone || "";
    this.user_birthday = this.userInfo.birthday || "";
    this.user_avatar_imgUrl = this.userInfo.avatar || "";
    this.user_avatar = this.userInfo.avatar || "";
  },
  computed: {
    ...mapState(["userInfo"]),
  },
  methods: {
    ...
    async saveUserInfo() {
      ...
      if (this.user_avatar) {
        ...
        userdata.avatar = this.user_avatar;
      }

      let result = await changeUserInfo(userdata);
      // 3.3 返回主界面
      if (result) {
        // 3.2 提示用户
        this.$message({
        ...
        // 3.4 更新本地数据
        this.$store.dispatch("getUserInfo", { id: this.userInfo.id });
        // 3.5 返回主界面
        setTimeout(() => {
