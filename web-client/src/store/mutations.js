...
export default {
  ...
  [HOME_SHOP_LIST](state, { homeshoplist }) {
    let index = 0;
    let newArray = [];
    while (index < homeshoplist.length) {
      newArray.push(homeshoplist.slice(index, index += 3));
    }
    // console.log(newArray);
    state.homeshoplist = newArray;
  },

  [RECOMMEND_SHOP_LIST](state, { recommendshoplist }) {
    // console.log(recommendshoplist);
    state.recommendshoplist = recommendshoplist.records;
  },
